...
public:
	...
	ENDPOINT_INFO(queryJobNameList) {
		// 定义标题和返回类型以及授权支持
		API_DEF_ADD_COMMON_AUTH(ZH_WORDS_GETTER("jobnamelist.query-jobnamelist.summary"), JobNameListJsonVO::Wrapper);
		// 定义其他路径参数说明
		API_DEF_ADD_QUERY_PARAMS(String, "keyword", ZH_WORDS_GETTER("jobnamelist.field.keyword"), "", false);
	}
	// 3.2 定义测试声明式服务调用的接口1处理
	API_HANDLER_ENDPOINT_AUTH(API_M_GET, "/common/query-jobnamelist", queryJobNameList, QUERY(String, keyword), execQueryJobNameList(keyword, authObject->getPayload()));
private:
	// 3.3 测试声明式服务调用1
	JobNameListPageJsonVO::Wrapper execQueryJobNameList(const String& keyword, const PayloadDTO& payload);
};
