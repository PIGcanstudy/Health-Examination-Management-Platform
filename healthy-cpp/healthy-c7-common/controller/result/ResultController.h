...
#include"../../domain/dto/result/ItemResultDTO.h"
...
public: //定义接口
	...
	ENDPOINT_INFO(queryItemResult) {
		// 定义接口标题
		API_DEF_ADD_TITLE(ZH_WORDS_GETTER("result.query.detail"));
		// 定义默认授权参数（可选定义，如果定义了，下面ENDPOINT里面需要加入API_HANDLER_AUTH_PARAME）
		API_DEF_ADD_AUTH();
		// 定义响应参数格式
		...
		API_DEF_ADD_QUERY_PARAMS(String, "personId", ZH_WORDS_GETTER("result.field.personId"), "007", true);
		//组合项目
		API_DEF_ADD_QUERY_PARAMS(String, "orderGroupItemId", ZH_WORDS_GETTER("result.field.orderGroupItemId"),"madrid",true);
	}
	//2.2项目结论详情查询接口处理
	ENDPOINT(API_M_GET, "/result/depart-item-result", queryItemResult, QUERIES(QueryParams, params), API_HANDLER_AUTH_PARAME)
	{
		// 解析查询参数为Query领域模型（接收Browser层的参数，换成自己定义的Query类）
		API_HANDLER_QUERY_PARAM(resultQuery_2, ResultQuery, params);
		// 呼叫执行函数响应结果
		API_HANDLER_RESP_VO(execQueryPrimCheckDetail(resultQuery_2));
	}
private: //定义接口执行函数
	ResultPageJsonVO::Wrapper execQueryPrimCheck(const ResultQuery::Wrapper& query);
	ItemResultPageJsonVO::Wrapper execQueryPrimCheckDetail(const ResultQuery::Wrapper& query);
};
