...
public: // 定义接口
	...
	ENDPOINT_INFO(queryByPerson) {
		...
		// 定义响应参数格式
		API_DEF_ADD_RSP_JSON_WRAPPER(PersonPageJsonVO);
		// 定义分页查询参数描述
		API_DEF_ADD_PAGE_PARAMS();
		// 定义其他查询参数描述
		API_DEF_ADD_QUERY_PARAMS(String, "name", ZH_WORDS_GETTER("person.data.name"), ZH_WORDS_GETTER("person.example.name"), false);
	...
	// 定义查询接口处理
	ENDPOINT(API_M_GET, "/GPerson/Query/list", queryByPerson, QUERIES(QueryParams, queryParams)) {
