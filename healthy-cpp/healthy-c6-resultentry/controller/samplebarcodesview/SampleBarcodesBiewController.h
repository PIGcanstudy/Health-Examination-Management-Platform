...
class SampleBarcodesViewController :public oatpp::web::server::api::ApiController // 1 继承控制器
{
	...
	// 3 定义接口
public:
	API_HANDLER_ENDPOINT_AUTH(API_M_GET, "healthy-c6-resultentry/barcod", querySampleBarcodesView, BODY_DTO(SampleBarcodesViewQuery::Wrapper,qdto), execQuerySampleBarcodesView(qdto));
...
private:
	StringJsonVO::Wrapper execQuerySampleBarcodesView(const SampleBarcodesViewQuery::Wrapper& qdto);
};
