    ...
    <el-card class="box-card">
      <template #header v-if="props?.useHeader">
        <div class="card-header">
          ...
          <el-button class="button" @click="operatingInstructionDialogVisible = true">
            <el-icon style="margin-right: 4px"> <icon-question /></el-icon>操作说明</el-button>
        </div>
      </template>
      <div class="main">
        <!-- 树形菜单插槽 -->
        ...
        <div class="theTable">
          ...
          <slot name="menu"></slot>
          <!-- 表格 -->
          <el-table :data="props?.tableData" style="width: 100%" @selection-change="handleSelectionChange"
            v-loading="openLoading" table-layout="auto">
            <!-- 带有多选功能的列 -->
            <el-table-column type="selection" v-if="props.useSelectColumn" width="55" />
            <el-table-column v-for="item in props?.tableColumnAttribute" :prop="item.prop" :label="item.label" :key="item"
              :sortable="item?.sortable" class-name="class-name">
              <!-- 表格的列内容如果使用tag -->
              ...
              <template #default="{ row }" v-else-if="item.useSwitch">
                <el-switch :model-value="row[item.prop] ? true : false" inline-prompt active-text="是" inactive-text="否"
                  size="large" :loading="openSwitchLoading" @change="(state) => emits('updateSwitchState', state, row)" />
              </template>
              <!-- 表格数据包含图片 -->
              <template #default="{ row }" v-else-if="item.usePic">
                <el-image style="width: 60px; height: 60px" :src="row[item.prop]" fit="fit" />
              </template>
            </el-table-column>
            <el-table-column label="操作" v-if="!props?.useDropdownMenu && props.useOperateColumn" fixed="right">
              <!-- 带图标的按钮操作 -->
              <template #default="{ row }">
                <!-- 条件渲染按钮 -->
                <template v-if="props.handleEdit">
                  <el-tooltip content="编辑" placement="top">
                    <el-button plain circle type="primary" @click="props.handleEdit(row)">
                      <el-icon> <icon-edit /></el-icon>
                    </el-button> </el-tooltip></template>
                <!-- 条件渲染按钮 -->
                <template v-if="props.handleDelete">
                  <el-tooltip content="删除" placement="top"><el-button plain circle type="danger"
                      @click="props.handleDelete(row)">
                      <el-icon>
                        <icon-delete /></el-icon> </el-button></el-tooltip></template>
              </template>
            </el-table-column>
            <el-table-column label="操作" v-else-if="props.useDropdownMenu && props.useOperateColumn" fixed="right">
              <!-- 下拉菜单的按钮操作 -->
              <template #default="{ row }">
                <el-dropdown trigger="click" @command="(command) => handleCommand(command, row)">
                  <el-button>
                    操作
                    <el-icon style="margin-left: 4px"><icon-caret-bottom /></el-icon>
                  </el-button>
                  <template #dropdown>
                    <el-dropdown-menu>
                      <el-dropdown-item v-for="(item, index) in props?.dropdownMenuActionsInfo" :key="index"
                        :command="item.command">{{ item.actionName }}</el-dropdown-item>
                    </el-dropdown-menu>
                  </template>
      ...
      <template v-if="props?.usePagination">
        <el-pagination v-model:current-page="paginationData.currentPage" v-model:page-size="paginationData.pageSize"
          :page-sizes="props.pageSizes" layout="prev, pager, next, jumper, ->, total, sizes" :total="props.total"
          @size-change="handleSizeChange" @current-change="handleCurrentChange" style="margin-top: 30px" />
      </template>

      <!-- 操作说明的会话框 -->
      <el-dialog v-model="operatingInstructionDialogVisible" title="操作说明" width="50%">
        <span>{{ props.msg }}</span>
      </el-dialog>
    </el-card>
  </div>
